...
function PrimitivePage({ primitive, pages }: PrimitivePageProps) {
	...
	return <>
		<h1>{primitive.planned && <PlannedBadge />}{primitive.deprecated && <DeprecatedBadge />}<code>{primitive.glyph}</code> {primitive.name} {otherPrimitives.length ? <div class='dropdown d-inline-block'>
