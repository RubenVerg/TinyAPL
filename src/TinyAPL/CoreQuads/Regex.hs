...
regex = Nilad (Just $ do
  scope <- createRef $ Scope [] [ ("Search", (VariableConstant, PrimitiveFunction Nothing (Just $ const search') Nothing Nothing Nothing Nothing Nothing Nothing "Search" Nothing)) ] [] [] Nothing
  ctx <- getContext
