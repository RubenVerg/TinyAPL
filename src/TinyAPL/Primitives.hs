...
minus = PrimitiveFunction (Just $ const F.neg') (Just $ const F.sub') (Just $ const F.neg') (Just $ const F.sub') (Just $ const F.add') Nothing Nothing Nothing [G.minus] Nothing
...
divide = PrimitiveFunction (Just $ const F.reciprocal') (Just $ const F.divide') (Just $ const F.reciprocal') (Just $ const F.divide') (Just $ const F.times') Nothing Nothing Nothing [G.divide] Nothing
power = PrimitiveFunction (Just $ const F.ePow') (Just $ const F.pow') (Just $ const F.ln') (Just $ const F.log') (Just $ const $ F.commute F.root') Nothing Nothing Nothing [G.power] Nothing
logarithm = PrimitiveFunction (Just $ const F.ln') (Just $ const F.log') (Just $ const F.ePow') (Just $ const F.pow') (Just $ const $ F.root') Nothing Nothing Nothing [G.logarithm] Nothing
root = PrimitiveFunction (Just $ const F.squareRoot') (Just $ const F.root') (Just $ const F.square') (Just $ const F.raises') (Just $ const F.log') Nothing Nothing Nothing [G.root] Nothing
...
notIdentical = PrimitiveFunction (Just $ const F.tally') (Just $ withCoreExtraArgs2 F.notIdentical') (Just $ const F.unTally') Nothing Nothing Nothing Nothing Nothing [G.notIdentical] Nothing
rho = PrimitiveFunction (Just $ const F.shape') (Just $ const F.reshape') (Just $ withCoreExtraArgs1 F.unShape') Nothing Nothing Nothing Nothing Nothing [G.rho] Nothing
...
drop = PrimitiveFunction (Just $ const F.majorCells') (Just $ withCoreExtraArgs2 F.drop') (Just $ withCoreExtraArgs1 F.mix) Nothing Nothing Nothing Nothing Nothing [G.drop] Nothing
left = PrimitiveFunction (Just $ \_ x -> pure x) (Just $ \_ x _ -> pure x) (Just $ \_ x -> pure x) Nothing (Just $ \_ x _ -> pure x) Nothing Nothing Nothing [G.left] Nothing
right = PrimitiveFunction (Just $ \_ x -> pure x) (Just $ \_ _ y -> pure y) (Just $ \_ x -> pure x) (Just $ \_ _ y -> pure y) Nothing Nothing Nothing Nothing [G.right] Nothing
iota = PrimitiveFunction (Just $ withCoreExtraArgs1 F.indexGenerator') (Just $ withCoreExtraArgs2 F.indexOf) Nothing (Just $ withCoreExtraArgs2 $ F.commute . F.from) Nothing Nothing Nothing Nothing [G.iota] Nothing
indices = PrimitiveFunction (Just $ withCoreExtraArgs1 F.indices) (Just $ withCoreExtraArgs2 F.intervalIndex) (Just $ withCoreExtraArgs1 F.histogram) Nothing Nothing Nothing Nothing Nothing [G.indices] Nothing
replicate = PrimitiveFunction Nothing (Just $ const F.replicate') Nothing Nothing Nothing (Just $ const F.runLengthEncode') Nothing Nothing [G.replicate] Nothing
...
phase = PrimitiveFunction (Just $ const F.phase') (Just $ const F.arctan') (Just $ const F.unitPolar') Nothing Nothing (Just $ const F.sineAndCosine') Nothing Nothing [G.phase] Nothing
...
difference = PrimitiveFunction (Just $ const F.not') (Just $ withCoreExtraArgs2 F.difference') (Just $ const F.not') Nothing Nothing Nothing Nothing Nothing [G.difference] Nothing
...
rank = PrimitiveFunction (Just $ const F.rank') (Just $ const F.rerank') (Just $ withCoreExtraArgs1 F.unRank') Nothing Nothing Nothing Nothing Nothing [G.rank] Nothing
...
transpose = PrimitiveFunction (Just $ withCoreExtraArgs1 F.transpose) (Just $ withCoreExtraArgs2 F.dyadicTranspose') (Just $ withCoreExtraArgs1 $ \cea -> F.transpose cea{ coreExtraArgsBackward = not $ coreExtraArgsBackward cea }) (Just $ withCoreExtraArgs2 $ \cea -> F.dyadicTranspose' cea{ coreExtraArgsBackward = not $ coreExtraArgsBackward cea }) Nothing Nothing Nothing Nothing [G.transpose] Nothing
matrixInverse = PrimitiveFunction (Just $ const F.matrixInverse') (Just $ const F.matrixDivide') (Just $ const F.matrixInverse') (Just $ const F.matrixDivide') (Just $ const $ F.commute $ F.innerProduct (F.reduce' defaultCoreExtraArgs F.add') F.times') Nothing Nothing Nothing [G.matrixInverse] Nothing
...
decode = PrimitiveFunction (Just $ withCoreExtraArgs1 F.decodeBase2) (Just $ withCoreExtraArgs2 F.decode') (Just $ withCoreExtraArgs1 F.encodeBase2) (Just $ withCoreExtraArgs2 F.encode') Nothing Nothing Nothing Nothing [G.decode] Nothing
encode = PrimitiveFunction (Just $ withCoreExtraArgs1 F.encodeBase2) (Just $ withCoreExtraArgs2 F.encode') (Just $ withCoreExtraArgs1 F.decodeBase2) (Just $ withCoreExtraArgs2 F.decode') Nothing Nothing Nothing Nothing [G.encode] Nothing
histogram = PrimitiveFunction (Just $ withCoreExtraArgs1 F.histogram) (Just $ withCoreExtraArgs2 F.count) (Just $ withCoreExtraArgs1 F.indices) Nothing Nothing Nothing Nothing Nothing [G.histogram] Nothing
increment = PrimitiveFunction (Just $ const F.increment') Nothing (Just $ const F.decrement') Nothing Nothing Nothing Nothing Nothing [G.increment] Nothing
decrement = PrimitiveFunction (Just $ const F.decrement') (Just $ const F.span') (Just $ const F.increment') (Just $ const F.span') (Just $ const $ F.atop F.decrement' F.add') Nothing Nothing Nothing [G.decrement] Nothing
...
execute = PrimitiveFunction (Just $ const F.execute') (Just $ const F.executeWith) (Just $ const $ \y -> vector . fmap Character <$> showM (Repr y)) Nothing Nothing Nothing Nothing Nothing [G.execute] Nothing
...
over = PrimitiveConjunction
  ...
  , conjContext = Nothing
  , conjOnNounNoun = Nothing
  , conjOnNounFunction = Just $ \_ x g -> pure $ DerivedFunctionNounFunction (Just $ \_ y -> callDyad g [] x y) (Just $ \_ x' y -> callDyad g [] x' y) Nothing Nothing Nothing Nothing Nothing Nothing Nothing over x g
  , conjOnFunctionNoun = Just $ \_ f y -> pure $ DerivedFunctionFunctionNoun (Just $ \_ x -> callDyad f [] x y) (Just $ \_ x y' -> callDyad f [] x y') Nothing Nothing Nothing Nothing Nothing Nothing Nothing over f y
...
reverseAtop = PrimitiveConjunction
  ...
  , conjContext = Nothing
  , conjOnNounNoun = Nothing
  , conjOnNounFunction = Just $ \_ y g -> pure $ DerivedFunctionNounFunction (Just $ \_ x -> callDyad g [] x y) Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing reverseAtop y g
  , conjOnFunctionNoun = Just $ \_ f x -> pure $ DerivedFunctionFunctionNoun (Just $ \_ y -> callDyad f [] x y) Nothing Nothing Nothing Nothing Nothing Nothing Nothing Nothing reverseAtop f x
...
reverseOver = PrimitiveConjunction
  ...
  , conjContext = Nothing
  , conjOnNounNoun = Nothing
  , conjOnNounFunction = Just $ \_ y g -> pure $ DerivedFunctionNounFunction (Just $ \_ x -> callDyad g [] x y) (Just $ \_ x y' -> callDyad g [] x y') Nothing Nothing Nothing Nothing Nothing Nothing Nothing reverseOver y g
  , conjOnFunctionNoun = Just $ \_ f x -> pure $ DerivedFunctionFunctionNoun (Just $ \_ y -> callDyad f [] x y) (Just $ \_ x' y -> callDyad f [] x' y) Nothing Nothing Nothing Nothing Nothing Nothing Nothing reverseOver f x
...
innerProduct = PrimitiveConjunction
  ...
  , conjOnNounFunction = Nothing
  , conjOnFunctionNoun = Nothing
  , conjOnFunctionFunction = Just $ \ea f g -> pure $ DerivedFunctionFunctionFunction (Just $ \ea' -> withCoreExtraArgs1 (rollR F.alternant (callMonad f []) (callDyad g [])) (ea' ++ ea)) (Just $ const $ F.innerProduct (callMonad f []) (callDyad g [])) Nothing Nothing Nothing Nothing Nothing Nothing Nothing innerProduct f g }
