...
doFFI libName args' = do
  ...
  pure $ scalar $ Wrap $ PrimitiveFunction (Just $ const $ \y -> do
    ...
    ffiFunc sym params' ret' args) Nothing Nothing Nothing Nothing Nothing (case libName of
...
ffi = PrimitiveFunction (Just $ const $ doFFI Nothing) (Just $ const $ \x y -> do
  ...
  doFFI (Just name) y) Nothing Nothing Nothing Nothing (G.quad : "FFI") Nothing
...
pointer typ ptr = do
  ...
  scope <- createRef $ Scope
    ...
    [ ("Peek", (VariableConstant, PrimitiveFunction (Just $ const $ scalarMonad $ \y -> do
      let err = DomainError "Pointer peek offset must be an integer"
      offset <- asNumber err y >>= asInt err
      fmap toScalar $ peek $ plusPtr ptr $ offset * size) Nothing Nothing Nothing Nothing Nothing "Peek" Nothing))
    , ("PeekB", (VariableConstant, PrimitiveFunction (Just $ const $ scalarMonad $ \y -> do
      let err = DomainError "Pointer peek offset must be an integer"
      offset <- asNumber err y >>= asInt err
      fmap toScalar $ peek $ plusPtr ptr offset) Nothing Nothing Nothing Nothing  Nothing "PeekB" Nothing))
    , ("PeekL", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      ...
      ffiPeekArrayLen typ (plusPtr ptr $ offset * size) $ fromIntegral len) Nothing Nothing Nothing Nothing  "PeekL" Nothing))
    , ("PeekE", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      ...
      ffiPeekArrayEnd typ (plusPtr ptr $ offset * size) y) Nothing Nothing Nothing Nothing  "PeekE" Nothing))
    , ("Poke", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> poke ptr y $> vector []) (Just $ const $ \x y -> do
      ...
      offset <- asScalar err x >>= asNumber err >>= asInt err
      poke (plusPtr ptr $ offset * size) y $> vector []) Nothing Nothing Nothing Nothing  "Poke" Nothing))
    , ("PokeB", (VariableConstant, PrimitiveFunction Nothing (Just $ const $ \x y -> do
      let err = DomainError "Pointer poke offset must be an integer"
      offset <- asScalar err x >>= asNumber err >>= asInt err
      poke (plusPtr ptr offset) y $> vector []) Nothing Nothing Nothing Nothing "PokeB" Nothing))
    , ("PokeA", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> ffiPokeArray typ ptr y $> vector []) Nothing Nothing Nothing Nothing Nothing "PokeA" Nothing))
    , ("PokeE", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> ffiPokeArray0 typ ptr y $> vector []) (Just $ const $ \x y -> ffiPokeArrayEnd typ ptr x y $> vector []) Nothing Nothing Nothing Nothing "PokeE" Nothing))
    , ("AddB", (VariableConstant, PrimitiveFunction (Just $ const $ scalarMonad $ \y -> do
      let err = DomainError "Pointer add byte offset must be an integer"
      offset <- asNumber err y >>= asInt err
      pointer typ $ plusPtr ptr offset) Nothing Nothing Nothing Nothing Nothing  "AddB" Nothing))
    , ("SubB", (VariableConstant, PrimitiveFunction (Just $ const $ scalarMonad $ \case
      (Number off) -> do
      ...
      _ -> throwError $ DomainError "Invalid arguments to pointer subtract byte") Nothing Nothing Nothing Nothing Nothing "SubB" Nothing))
    , ("Cast", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      ...
      case parseFFI typ of
        ...
        Just ffiTyp -> scalar <$> pointer ffiTyp ptr) Nothing Nothing Nothing Nothing Nothing "Cast" Nothing))
    , (G.deltaBar : "Add", (VariableConstant, PrimitiveFunction Nothing (Just $ const $ \cases
      ...
      (Array [] [Number off]) (Array [] [pointerObj@(Struct _)]) -> do
        ...
        p <- pointerGet pointerObj
        scalar <$> pointer typ (plusPtr p $ offset * size)
      _ _ -> throwError $ DomainError "Invalid arguments to pointer add") Nothing Nothing Nothing Nothing (G.deltaBar : "Add") Nothing)) 
    , (G.deltaBar : "Subtract", (VariableConstant, PrimitiveFunction Nothing (Just $ const $ \cases
      ...
      _ _ -> throwError $ DomainError "Invalid arguments to pointer subtract") Nothing Nothing Nothing Nothing (G.deltaBar : "Subtract") Nothing))
    , (G.deltaBar : "Increment", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      sc <- asScalar undefined y
      p <- pointerGet sc
      scalar <$> pointer typ (plusPtr p 1)) Nothing Nothing Nothing Nothing Nothing (G.deltaBar : "Increment") Nothing))
    , (G.deltaBar : "Decrement", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      sc <- asScalar undefined y
      p <- pointerGet sc
      scalar <$> pointer typ (plusPtr p $ -1)) Nothing Nothing Nothing Nothing Nothing (G.deltaBar : "Decrement") Nothing))
    , (G.deltaBar : "Sign", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      sc <- asScalar undefined y
      p <- pointerGet sc
      pure $ scalar $ boolToScalar $ castPtr p == nullPtr) Nothing Nothing Nothing Nothing Nothing (G.deltaBar : "Sign") Nothing)) ]
    [] [] Nothing
  ctx <- getContext
...
ffiStruct = Nilad (Just $ do
  ...
  scope <- createRef $ Scope
    ...
    [ ("New", (VariableConstant, PrimitiveFunction Nothing (Just $ const $ \x y -> do
      typ <- asString (DomainError "FFI New left argument must be a string") x
      case parseFFI typ of
        Nothing -> throwError $ DomainError "Invalid New type"
        Just typ' -> ffiNew typ' y >>= fmap scalar . pointer typ' . fst) Nothing Nothing Nothing Nothing "New" Nothing))
    , ("NewA", (VariableConstant, PrimitiveFunction Nothing (Just $ const $ \x y -> do
      typ <- asString (DomainError "FFI NewA left argument must be a string") x
      case parseFFI typ of
        Nothing -> throwError $ DomainError "Invalid NewA type"
        Just typ' -> ffiNewArray typ' y >>= fmap scalar . pointer typ' . fst) Nothing Nothing Nothing Nothing "NewA" Nothing))
    , ("NewZ", (VariableConstant, PrimitiveFunction Nothing (Just $ const $ \x y -> do
      typ <- asString (DomainError "FFI NewZ left argument must be a string") x
      case parseFFI typ of
        Nothing -> throwError $ DomainError "Invalid NewZ type"
        Just typ' -> ffiNewArray0 typ' y >>= fmap scalar . pointer typ' . fst) Nothing Nothing Nothing Nothing "NewZ" Nothing))
    , ("Alloc", (VariableConstant, PrimitiveFunction (Just $ const $ doAlloc 1) (Just $ const $ \x y -> do
      let err = DomainError "FFI Alloc left argument must be a natural"
      count <- asScalar err x >>= asNumber err >>= asNat err
      doAlloc (fromIntegral count) y) Nothing Nothing Nothing Nothing "Alloc" Nothing))
    , ("Free", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      ptr <- asScalar (DomainError "FFI Free argument must be a pointer object") y >>= pointerGet
      liftIO $ free ptr
      pure $ vector []) Nothing Nothing Nothing Nothing Nothing "Free" Nothing))
    , ("SizeOf", (VariableConstant, PrimitiveFunction (Just $ const $ \y -> do
      ...
      case mapM parseFFI typs of
        ...
        Just typs' -> pure $ Array sh $ Number . (:+ 0) . fromIntegral . ffiSize <$> typs') Nothing Nothing Nothing Nothing Nothing "SizeOf" Nothing)) ]
    [] [] Nothing
  ctx <- getContext
